...
class ArucoPoseEstimation():
	...
	def __init__(self):
		...
		#self.distortion = np.array((-0.43948,0.18514,0,0,0))
	...
	def aruco_process_image_pi(self):
		...
		output = self.pose_estimation(img, ARUCO_DICT[self.aruco_type], self.intrinsic_camera, self.distortion)
	...
	def pose_estimation(self, frame, aruco_dict_type, matrix_coefficients, distortion_coefficients):
		...
		if len(corners) > 0:
			...
			for i in range(0, len(self.ids)):
				...
				self.rvec, self.tvec, markerPoints = cv2.aruco.estimatePoseSingleMarkers(corners[i], 0.105 , matrix_coefficients,
